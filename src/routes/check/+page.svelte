...
<style>
  .check-container { padding: 60px 20px; display: flex; justify-content: center; }
  .search-box { width: 100%; max-width: 700px; text-align: center; }
  
  h1 { font-weight: 900; font-size: 3.5rem; letter-spacing: -2px; line-height: 1.1; }
  .blue-text { color: #2563eb; }
  
  .input-row { display: flex; gap: 15px; margin: 40px 0; }
  input { flex: 1; padding: 20px; border: 4px solid black; border-radius: 15px; font-weight: 700; font-size: 1.1rem; width: 100%; }
  button { background: #2563eb; color: white; border: 4px solid black; padding: 0 40px; border-radius: 15px; font-weight: 900; cursor: pointer; box-shadow: 4px 4px 0px black; transition: transform 0.1s; }
  ...
  .card-content-wrapper { display: flex; align-items: center; gap: 20px; }
  ...
  .date { font-size: 1.4rem; font-weight: 900; white-space: nowrap; }
  ...
  @media (max-width: 600px) {
    h1 { font-size: 2.5rem; }
    .check-container { padding: 40px 15px; }
    
    .input-row { flex-direction: column; gap: 10px; }
    button { padding: 18px; width: 100%; }
    
    .capsule-card { flex-direction: column; align-items: flex-start; gap: 15px; }
    .action-area { width: 100%; }
    .status-btn.unlocked, .status-badge.locked { display: block; text-align: center; width: 100%; }
  }
</style>